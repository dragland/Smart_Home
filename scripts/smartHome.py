...
def write_archive():
	...
	curs = conn.cursor()
	curs.execute("INSERT INTO data values((?), (?), (?), (?), (?), (?), (?), (?))", (datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), config.temp_f, config.rh, config.door, config.lights_red, config.lights_green, config.lights_blue, config.fan))
	conn.commit()
	conn.close()
...
def init_archive():
	conn = sqlite3.connect("html/archive.db")
	curs = conn.cursor()
	curs.execute("CREATE TABLE IF NOT EXISTS data (timestap DATETIME, temp_f NUMERIC, rh NUMERIC, door INTEGER, lights_red INTEGER, lights_green INTEGER, lights_blue INTEGER, fan INTEGER)")
	conn.commit()
	conn.close()